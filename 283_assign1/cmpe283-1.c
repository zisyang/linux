...
detect_vmx_features(void)
{
	...
	procbased_hi = hi; // save for checking procbased2
	pr_info("Procbased Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procbased, 21, lo, hi);

	/* Secondary Procbased controls 
	 *  1st to check the ability to set “Activate Secondary Controls” 
	 *  control in the primary procbased controls 
	 */
	if (procbased_hi & (1 << (&procbased[20])->bit))
	{
	...
	{	//if the bit cannot be set 
		pr_info("Secondary Procbased Controls NOT Available");
	}
	...
	pr_info("Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(exitctl, 11, lo, hi);
	
	/* Entry controls */
	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("Entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(entryctl, 9, lo, hi);
}
