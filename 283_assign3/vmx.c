...
// 283 assignment 3:
// exits_per_reason: for function __vmx_handle_exit(...) & kvm_emulate_cpuid(..)
...
static int __vmx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t exit_fastpath)
{
	...
	total_exits_all_types++; // increment the total exit for all types

	exits_per_reason[exit_reason.basic]++; // increment the exit counter per exit reason
	//arch_atomic_inc(&exits_per_reason[exit_reason.basic]); // or for atomic
...
// 283 assignment 3:
// total_time_spent_per_reason: for function vmx_handle_exit(...) & kvm_emulate_cpuid(..)
//  For CPUID leaf node %eax=0x4FFFFFFC:   Return the time spent processing the exit number provided (on input) in %ecx
extern u64 total_time_spent_per_reason[70]; // atomic_long_t total_time_spent_per_reason[70];
